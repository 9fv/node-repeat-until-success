...
class RepeatTaskUntilSuccess {
  ...
  run() {
    ...
    p = p.then((value) => {
      ...
      if ((err instanceof Promise.TimeoutError) || (this.end - this.start >= this._timeout)) {
        throw err;
      }
      ...
      if (this._timeout) {
        result = result.timeout(this._timeout);
      }
      this.end = new Date();
      result = result.then(() => {
        return this.run();
 ...
 *
 * @param task {Function} - The function to repeat.
 * @param args {array} - Array of arguments passed when calling task.
